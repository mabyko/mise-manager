...
### Changed
- 메뉴바 앱으로 전환(`LSUIElement`). Dock 아이콘은 메인 창이 열려 있는 동안에만 보이고, 창을 닫으면 메뉴바에서만 실행된다. 메뉴바 아이콘을 끈 경우에는 접근 경로를 위해 Dock 아이콘을 유지한다
- 설정에 **로그인 시 시작** 추가(`SMAppService.mainApp`). 로그인 항목으로 실행되면 창 없이 메뉴바에만 올라온다

### Fixed
- 메뉴바 창의 상단 톱니바퀴(설정 열기)가 눌리지 않던 문제. 숨긴 타이틀바 영역의 mouse-down이 패널을 닫아 버튼이 실행되지 않았다(`isMovable = false`)
