...
PoC 검증을 완료하고 0.1.0에서는 실사용 가능한 설치/업데이트 관리 흐름을 정리한다.

## 2. 핵심 가치
- 여러 도구의 설치/전역 사용 상태를 한 번에 파악
- 업데이트 의사결정을 분기(같은 메이저/릴리즈/프리릴리즈)
- 설치/전환/삭제 작업을 클릭 중심으로 실행
- plugin definition의 기본 소스와 custom URL 상태를 시각화
...
## 8. 차기 우선순위(초안)
1. Installs 탭 상세 개선(정렬/필터/URL validation)
2. Updater 탭 상태 라벨 세분화(Up-to-date/Installed-only 등)
3. e2e 회귀 체크 자동화
4. 릴리즈/패키징 플로우 확정
